...
function showCheckoutAlert() {
    ...
    if (address !== "[Address not set]") {
        ...
        alert("Please provide a delivery address to proceed with the checkout. Check at your profile");
    }
}
...
window.onload = () => {
    ...
    const sex = localStorage.getItem("sex") || "[Not Set]";
    const address = localStorage.getItem("address") || "[Not Set]";
    ...
    // Check if 30 seconds have passed since the last alert
    if (username === "[Not Set]" || sex === "[Not Set]" || address === "[Not Set]") {
        if (currentTime - lastAlertTime >= 30000) { // 30 seconds
            // Show alert
            alert("Welcome! Please set up your profile to complete your information. Click on your profile at the sidebar to set it up.");
